...
const translate = async (query, enko = false, google = false) => {
    ...
    try {
        return await (papago === null || papago === void 0 ? void 0 : papago.translate(query, enko));
    }
    catch (error) {
...
const lookup = async (query, { enko = false, google = false, strip = false } = {}) => {
    ...
    const combinedExamples = await utils_1.getCombinedExamples(result, { fetchExtraData: true, strip });
